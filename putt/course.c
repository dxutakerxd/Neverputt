...
static int course_load(struct course *crs, const char *filename)
{
    ...
    if ((fin = fs_open_read(filename)))
    {
            ...
            fs_gets(crs->desc, sizeof (crs->desc), fin))
        {
            ...
            /* Read optional name line (3rd header line). */

            if (fs_gets(crs->name, sizeof (crs->name), fin))
            {
                strip_newline(crs->name);

                /* If it looks like hole data, it's not a name. */

                if (strstr(crs->name, ".sol"))
                    crs->name[0] = '\0';
            }
...
{
    return course_exists(i) ? course_v[i].name : "";
}
