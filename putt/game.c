...
void game_free_draw(void)
{
    if (state)
        sol_free_draw(&file.draw);
}

void game_load_draw(void)
{
    if (state)
                                                 ...
                                                 const struct s_vary *vp)
{
    ...
    if (vp->xv)
        for (i = 0; i < vp->xc; i++)
        {
            ...
            int sf = (xp->f == 0 || xp->f == 1) ? xp->f : 0;
            int se = (xp->e == 0 || xp->e == 1) ? xp->e : 0;

            if (!xp->base->i)
                beam_draw(rend, xp->base->p, swch_c[sf][se],
                                xp->base->r, 2.0f);
        }
}
