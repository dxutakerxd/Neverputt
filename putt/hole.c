...
static int hole_start = 1;   /* first playable hole (1-based) */
static int hole_end   = 0;   /* last playable hole (0 = use count-1, i.e. all) */
...
static void hole_init_rc(const char *filename)
{
    ...
    {
        /* Skip shot, description, and name. */
...
void hole_set_range(int start, int end)
{
    hole_start = start;
    hole_end   = end;
}

int hole_get_start(void)
{
    return hole_start;
}
...
int hole_move(void)
{
    int limit = hole_end > 0 ? hole_end : count - 1;
...
void hole_fall_undo(void)
{
    /* Undo the +2 penalty from hole_fall(). Ball position already reset. */
    score_v[hole][player] -= 2;

    /* If score cap was triggered, undo done/stat_v. */
    if (stat_v[player])
    {
        stat_v[player] = 0;
        done--;
