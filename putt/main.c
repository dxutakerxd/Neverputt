...
static int loop(void)
{
    ...
    while (d && SDL_PollEvent(&e))
    {
        ...
        case SDL_FINGERDOWN:
        case SDL_FINGERUP:
            /* Convert normalized touch coordinates to window coordinates. */

            ax = (int)(e.tfinger.x * video.window_w);
            ay = (int)(e.tfinger.y * video.window_h);

            /* Convert to OpenGL coordinates. */

            ax = ROUND(+ax * video.device_scale);
            ay = ROUND((-ay + video.window_h) * video.device_scale);

            /* Update cursor position, then send click. */

            st_point(ax, ay, 0, 0);
            d = st_click(SDL_BUTTON_LEFT, e.type == SDL_FINGERDOWN ? 1 : 0);
            break;
...
int main(int argc, char *argv[])
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_JOYSTICK) == 0)
    {
        ...
        /* Apply touch sensitivity mapping. */
        config_set_d(CONFIG_MOUSE_SENSE,
                     600 - (config_get_d(CONFIG_TOUCH_SENSE) - 1) * 50);
#endif

#ifdef ENABLE_WALLET
        /* If Seeker was previously verified, apply the ball skin. */

        if (config_get_d(CONFIG_WALLET_SEEKER))
        ...
        if (video_init())
        {
            ...
            /* Free game data while material cache is still valid. */
