...
#ifdef __MOBILE__
static int touch_id[10];
static int haptic_on_id;
static int haptic_off_id;
#endif
...
static int conf_action(int i)
{
    ...
    case CONF_HAPTIC_ON:
        config_set_d(CONFIG_HAPTIC, 1);
#ifdef __MOBILE__
        gui_set_hilite(haptic_on_id,  1);
        gui_set_hilite(haptic_off_id, 0);
#endif
        haptic_bump(0.5f);
        break;

    case CONF_HAPTIC_OFF:
        config_set_d(CONFIG_HAPTIC, 0);
#ifdef __MOBILE__
        gui_set_hilite(haptic_on_id,  0);
        gui_set_hilite(haptic_off_id, 1);
#endif
        break;
    ...
    default:
        ...
        if (300 <= i && i <= 309)
        {
            int old_ts = config_get_d(CONFIG_TOUCH_SENSE);
            int new_ts = i - 300 + 1;

            config_set_d(CONFIG_TOUCH_SENSE, new_ts);
            config_set_d(CONFIG_MOUSE_SENSE, 600 - (new_ts - 1) * 50);
            audio_play(AUD_BUMP, 1.f);

            gui_toggle(touch_id[new_ts - 1]);
            gui_toggle(touch_id[old_ts - 1]);
        }
#endif
    }
...
static int conf_enter(struct state *st, struct state *prev)
{
    ...
    if ((id = gui_vstack(0)))
    {
        ...
        if ((jd = gui_harray(id)) &&
            (kd = gui_harray(jd)))
        {
            int ts = config_get_d(CONFIG_TOUCH_SENSE);

            for (i = 10; i >= 1; i--)
            {
                touch_id[i - 1] = gui_state(kd, NULL, GUI_SML, 300 + i - 1, 0);
                gui_set_hilite(touch_id[i - 1], (ts == i));
            }

            gui_label(jd, _("Sensitivity"), GUI_SML, 0, 0);
        }

        if ((jd = gui_harray(id)) &&
            (kd = gui_harray(jd)))
        {
            int h = config_get_d(CONFIG_HAPTIC);

            haptic_off_id = gui_state(kd, _("Off"), GUI_SML, CONF_HAPTIC_OFF, 0);
            haptic_on_id  = gui_state(kd, _("On"),  GUI_SML, CONF_HAPTIC_ON,  0);

            gui_set_hilite(haptic_on_id,  h);
            gui_set_hilite(haptic_off_id, !h);

            gui_label(jd, _("Haptic"), GUI_SML, 0, 0);
        }
#endif
