...
void audio_init(void)
{
    ...
    /* Reset stale pointers (process may survive across Android relaunches). */
...
void audio_free(void)
{
    ...
    /* Free all voice structures. */

    if (music) { voice_free(music); music = NULL; }
    if (queue) { voice_free(queue); queue = NULL; }
