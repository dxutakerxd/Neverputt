...
static int sol_count_geom(const struct s_base *base, int g0, int gc, int mi)
{
    ...
    for (gi = 0; gi < gc; gi++)
    {
        ...
        if (idx >= 0 && idx < base->gc && base->gv[idx].mi == mi)
            c++;
    }
...
{
    const struct b_texc *tq;
    const struct b_side *sq;
    const struct b_vert *vq;
    int ti, si, vi;

    /* Bounds-check indirect indices from the offs array. */
                          ...
                          const struct s_base *base, int *iv, int g0, int gc, int mi)
{
    ...
    {
        int idx;
        const struct b_geom *gq;

        /* Bounds-check the index array lookup to avoid heap corruption     */
        /* from SOL files with shifted/corrupted index data.                */
        ...
        if (gq->mi == mi)
        {
            /* Bounds-check offs indices against the offs array size. */

            if (gq->oi < 0 || gq->oi >= base->oc ||
                gq->oj < 0 || gq->oj >= base->oc ||
                gq->ok < 0 || gq->ok >= base->oc)
                continue;
