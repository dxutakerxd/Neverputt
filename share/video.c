...
#ifdef __MOBILE__
        /* Approximate safe area insets for notches and rounded corners. */

        video.safe_t = video.device_h / 18;
        video.safe_b = video.device_h / 32;
        video.safe_l = video.device_w / 24;
        video.safe_r = video.device_w / 24;
#else
        video.safe_t = 0;
        video.safe_b = 0;
        video.safe_l = 0;
...
void video_push_persp(float fov, float n, float f)
{
    ...
    else
    {
        ...
        glMatrixMode(GL_PROJECTION);
        {
            ...
            if (a < 1.0f)
            {
                /* Portrait: treat fov as horizontal FOV. */
                m[0][0] =     c;
                m[1][1] = c * a;
            }
            else
            {
                /* Landscape: treat fov as vertical FOV. */
                m[0][0] = c / a;
                m[1][1] =     c;
            }
